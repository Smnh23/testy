...
@Slf4j
public class BotanicalFamilyReader extends MultiResourceItemReader<BotanicalFamilyData>{
	
	public BotanicalFamilyReader(String repositoryPath) throws FileNotFoundException {
	...
	private FlatFileItemReader<BotanicalFamilyData> readOneFile() {	
		FlatFileItemReader<BotanicalFamilyData> reader = new FlatFileItemReader<BotanicalFamilyData>();
		reader.setLinesToSkip(1);
    reader.setLineMapper(lineMapper());
		return reader;
	}
	
  public LineMapper<BotanicalFamilyData> lineMapper() {
    DefaultLineMapper<BotanicalFamilyData> lineMapper = new DefaultLineMapper<BotanicalFamilyData>();
    DelimitedLineTokenizer lineTokenizer = new DelimitedLineTokenizer();
    lineTokenizer.setNames(new String[] { "id", "name", "description" });
    lineTokenizer.setIncludedFields(new int[] { 0, 1, 2 });
    lineTokenizer.setDelimiter(";");
    BeanWrapperFieldSetMapper<BotanicalFamilyData> fieldSetMapper = new BeanWrapperFieldSetMapper<BotanicalFamilyData>();
    fieldSetMapper.setTargetType(BotanicalFamilyData.class);
