...
@Slf4j
public class VarietyReader extends MultiResourceItemReader<VarietyData>{
	
	public VarietyReader(String repositoryPath) throws FileNotFoundException {
	...
	private FlatFileItemReader<VarietyData> readOneFile() {	
		FlatFileItemReader<VarietyData> reader = new FlatFileItemReader<VarietyData>();
		reader.setLinesToSkip(1);
    reader.setLineMapper(lineMapper());
		return reader;
	}
	
  public LineMapper<VarietyData> lineMapper() {
    DefaultLineMapper<VarietyData> lineMapper = new DefaultLineMapper<VarietyData>();
    DelimitedLineTokenizer lineTokenizer = new DelimitedLineTokenizer();
    lineTokenizer.setNames(new String[] { "id", "idVegetable", "name", "hybridF1", "price", "planning" });
    lineTokenizer.setIncludedFields(new int[] { 0, 1, 2, 3, 4, 5 });
    lineTokenizer.setDelimiter(";");
    BeanWrapperFieldSetMapper<VarietyData> fieldSetMapper = new BeanWrapperFieldSetMapper<VarietyData>();
    fieldSetMapper.setTargetType(VarietyData.class);
